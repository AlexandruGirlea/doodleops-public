...
from app_pdf.views.v1.base_logic.pdf_merge_pdfs import (
...
@v1_view_pdf_router_openai.post(
	...
	description=(
			"Accepts a list of openaiFileIdRefs containing min two or max 10 pdf "
	...
	summary="Merge up to 10 PDFs into one PDF.",
)
async def view_pdf_merge_pdfs_openai(
		request: Request,
		...
		use_upload_order: bool = Query(
			False,
			description=(
					"Optional parameter. Ask the user if they want to merge the "
					"pdf files in the order they uploaded them or we should "
					"order them by name."
			)
		),
):
