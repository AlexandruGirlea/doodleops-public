...
@v1_view_pdf_router_openai.post(
	...
	description=(
			...
			"to extract all images and responds with a download link with the "
			"output zip file with all images. Max PDF file size is 10MB, and "
			"download links expire in 5 minutes."
	),
	summary="Extract images from PDF",
)
async def view_pdf_extract_images_openai(
		request: Request,
		...
		pages: str = Query(
			None,
			description=(
					"An optional parameter to specify page numbers, separated "
					"by commas, from which to extract images."
					)
		),
):
