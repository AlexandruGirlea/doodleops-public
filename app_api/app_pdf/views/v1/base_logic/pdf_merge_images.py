...
from common.file_validation import validate_file_type
...
APP_NAME, VERSION, API = "app_pdf", "v1", "view_pdf_merge_images"
...
async def get_cloud_run_response(
		token_data: TokenData, redis_conn: redis.Redis,
		files: list[UploadFile], use_upload_order: bool
):
	number_of_files = len(files)
	if number_of_files > URL_DATA.other["max_no_of_files"]:
		raise HTTPException(
			status_code=status.HTTP_400_BAD_REQUEST,
			detail=(
				f"You can upload a maximum of {URL_DATA.other['max_no_of_files']} "
				"images"
				)
		)
	...
	for file in files:
		validate_file_type(
			file=file, file_extensions=('png', 'jpg', 'jpeg',),
			content_type_startswith="image",
		)
	...
	try:
		...
		# validate all file sizes are under max_files_size_mb
		file_sizes = 0
