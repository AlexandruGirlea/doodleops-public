...
APP_NAME, VERSION, API = "app_pdf", "v1", "view_pdf_merge_pdfs"
...
async def get_cloud_run_response(
	...
	"""Merge PDFs into a single PDF based on the order of the uploaded files."""
	number_of_files = len(files)
	if number_of_files > URL_DATA.other["max_no_of_files"]:
		raise HTTPException(
			status_code=status.HTTP_400_BAD_REQUEST,
			detail=(
				f"You can merge a maximum of {URL_DATA.other['max_no_of_files']} "
				"PDFs"
			)
		)
	for file in files:
