...
def research_node(state: State) -> Command[Literal["research_node"]]:
	...
	perplexity_response = perplexity_search(
		system_msg=f"""
		You are an AI Handyman. Be direct, friendly, and provide concise
		responses.
		
		Based on the latest user prompt and the full conversation context, search
		the internet to help the user. Provide relevant
		information on topics such as (but not limited to):
		- research on home improvement projects
		- research on home maintenance
		- unit conversions
		- diy projects
		...
		tell the user to also consult a professional. You are an AI assistant that
		can make mistakes.
