...
response_schemas = [
	ResponseSchema(
		...
		description=(
			"Set to *True* if a valid media file (image: JPG, JPEG, PNG; or PDF) "
	...
	ResponseSchema(
		...
		description=(
			...
			"should be in {language}. If the user needs "
			"help with something that is potentially dangerous, advise the user "
			"to consult a professional, you are an AI assistant and not a "
			"licensed professional and you can make mistakes.`"
		)
	),
	ResponseSchema(
		...
		description=(
			...
			"(image or PDF) to process, as found in the conversation history or "
	...
	ResponseSchema(
		...
		description=(
			...
			"specify the supported formats (image: JPG, JPEG, PNG; or PDF). "
			"Example: `I couldn't identify the media file in your message. "
			"Please provide an image (JPG, JPEG, PNG) or a PDF so I can assist "
			"you.`\n"
			...
			"only process JPG, JPEG, PNG, or PDF files. Please upload one of "
			"these.`\n"
...
media_template = PromptTemplate(
	...
	template="""
		You are a friendly and helpful AI Cook. Your job is to help
		help the user.
		
		**Your Tasks:**
		- Examine the conversation history and the latest user message for a media
		file link (image: JPG, JPEG, PNG; or PDF) the user might be referencing.
		...
		"I can only process images (JPG, JPEG, PNG) and PDFs for now."
		...
		**Important:**
		...
		Tell the user to consult a professional if needed.
		- Ensure `media_link` matches the exact URL provided, without changes.
		""",
)


extract_requirements_chain = media_template | base_llm_vertexai | output_parser
...
def media_node(state: State) -> Command[Literal["media_node"]]:
	...
	if state.get("user_phone_number"):
		send_whatsapp_message(
			...
			body=translate(
				msg="Give me a few seconds to understand the image",
				language=language
			)
		)
