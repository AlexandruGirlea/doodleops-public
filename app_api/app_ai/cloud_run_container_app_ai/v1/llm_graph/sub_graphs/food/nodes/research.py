...
def research_node(state: State) -> Command[Literal["research_node"]]:
	...
	perplexity_response = perplexity_search(
		system_msg=f"""
		You are an AI Cook. Be direct, friendly, and provide concise
		responses.
		
		Based on the latest user prompt and the full conversation context, search
		the internet to help the user in his cooking journey. Provide relevant
		...
		If the user needs help with something that is potentially dangerous or
		toxic, tell the user to also consult a professional. You are an AI
