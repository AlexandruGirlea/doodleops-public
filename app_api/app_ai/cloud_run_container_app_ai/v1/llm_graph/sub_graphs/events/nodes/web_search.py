...
def web_search_node(state: State) -> Command[Literal["web_search_node"]]:
	...
	state["messages"] = [
		SystemMessage(
			content=f"""
				You are a helpful AI Events google search assistant.

				You job is to understand the conversation context, formulate the
				best google search query that will help the user find the events
				information he is looking for. Ex: events in `location`.
				
				In the query do not be too specific with the search
				terms, because it will affect the search results. You should try
				to formulate a general search query for events resources needed.
				For example if it's related up coming events search do something
				like this:
				- events in `location`
				- weekend events in `location`
				- upcoming events in `location`
				- rock concerts in `location`
				- music festivals in `location`
				- theater shows in `location`
				If the user want to plan or organize an event:
				- event planner in `location`
				- wedding planner in `location`
				...
				- find a wedding planner that costs 1000$ in `location`
				...
				If it's clear where the user is looking for events,
				ex: Paris, Tokyo, etc. You should change all these parameters
				`host_language_code`, `language_restrict`, `geolocation` to the
				country and language values. We want to search for the events
				resources the user wants, in the native language of the country.
				Also the Query should be in the language of the country.

				Example for France:
				- `query`: recherche d'un concert de rock
				- `host_language_code`: fr
				- `language_restrict`: lang_fr
				- `geolocation`: fr

				Example for USA:
				- `query`: looking for a rock concert
				...
				point to events resources the user is looking for. DO NOT provide
