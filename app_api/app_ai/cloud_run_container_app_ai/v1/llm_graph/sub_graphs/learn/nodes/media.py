...
media_template = PromptTemplate(
	...
	template="""
		You are a friendly and helpful AI learning assistant. Your job is to help
		help the user in his knowledge journey.
		...
		**Your Tasks:**
		...
		- The processor can extract text, describe images, solve problems, or
		answer questions based on the media. Include relevant context from the
		conversation history and latest message, as the processor lacks other
		context.
...
def media_node(state: State) -> Command[Literal["media_node"]]:
	...
	return Command(
		update={
			"messages": [
				AIMessage(content=msg, name="media_node", additional_kwargs=cost)
			]
		},
		goto=END
	)
