...
from app_docs.cloud_run_container_app_docs.v1.schemas import (
	view_docs_excel_remove_rows_based_on_condition as view_schema
)
from app_docs.views.v1.fastapi_views.route import v1_view_docs_router

logger = logging.getLogger(__name__)

APP_NAME, VERSION, API = (
	"app_docs", "v1", "view_docs_excel_remove_rows_based_on_condition"
)
API_NAME = "/".join([APP_NAME, VERSION, API])
URL_DATA: CloudRunAPIEndpoint = urls[APP_NAME][VERSION][API]


@v1_view_docs_router.post(
	URL_DATA.api_url,
	include_in_schema=True,
	name=URL_DATA.api_url.split("/")[-1],
)
async def view_docs_excel_remove_rows_based_on_condition(
	...
	"""
	Remove rows based on condition from an Excel file.

	We only allow one of these fields:
	remove_cell_values_same_row, remove_cell_values_different_rows,
	keep_cell_values_same_row, keep_cell_values_different_rows

	We have two additional Optional fields:
	`sheet_name` and `columns`
	"""
	view_schema.validate_get_form_data(data)
