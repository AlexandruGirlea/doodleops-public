...
from core.settings import GENERIC_ERROR_MSG, ISO_LANGUAGES
...
APP_NAME, VERSION, API = "app_images", "v1", "view_image_ocr"
...
async def view_image_ocr_docs(token_data: TokenData = Depends(verify_token)):
	return sorted(ISO_LANGUAGES, key=lambda x: x[0])


@v1_view_images_router.post(URL_DATA.api_url, include_in_schema=True)
async def view_image_ocr(
